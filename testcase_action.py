...
from testcase_element_exist import isElementExist, isInputExist, isUploadButton
...
class InitLinuxTrack(BaseConfigure):
    ...
    def select_device(self):
        ...
        test_device_name=config_data['testdevice']
...
def read_json_data():
    ...
    try:
        with open(file_path, 'r') as json_file:
            ...
            return data.get(testdevice),data.get(savedir)
...
def browser_configure():
    test_device_name, _ = read_json_data()
    if test_device_name is not None:
        test_device_name = test_device_name.replace("Jabra", "").replace(" ", "").lower()

    saveDir = read_json_data()[1] + "/" if read_json_data()[1] is not None else ""
    file = f"{saveDir}{test_device_name}" if test_device_name else ""
...
def rename_linux_zip(self, file, testcaseName, testDeviceName):
    ...
    try:
        ...
        print(testDeviceName + ' ' + 'testcase'+ testcaseName + ' download successful')
        self.close()
...
def setup_driver_windows(testcasename):
    ...
    testDeviceName = config_data.get('testdevice')
    savedir = config_data.get('savedir')

    if testDeviceName is not None and savedir is not None:
        file= os.path.join(savedir.replace('/', '\\'), testDeviceName.replace("Jabra", "").replace(" ", "").lower(), testcasename)
    else:
        print("Not testdevice/savedir.")
